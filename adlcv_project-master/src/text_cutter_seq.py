...
def process_text_files(input_dir, output_dir, patch_size=14, stride=4):
    ...
    for text_file in input_path.glob("*.txt"):
        ...
        for idx, ((x, y), patch) in enumerate(patches):
            path_output_dir = output_path / text_file.stem
            path_output_dir.mkdir(parents=True, exist_ok=True)
            patch_filename = path_output_dir / f"{text_file.stem}_{idx}.txt"
...
def main():
    ...
    stride = 14       # 4 characters = 64 pixels

    input_dir = data_dir / "raw_text"
    output_dir = data_dir / "processed_text/stride_224_text"
