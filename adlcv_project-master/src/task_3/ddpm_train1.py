...
def prepare_dataloader1(batch_size):
    import torchvision.transforms as transforms
    from torch.utils.data import DataLoader
    from dataset1.mario_dataset1 import MarioDataset
    transform = transforms.Compose([
    transforms.ToTensor(),                # from [0,255] to range [0.0,1.0]
    transforms.Normalize((0.5,), (0.5,))  # range [-1,1]

    ])
    dataset = MarioDataset(transform, num_samples=DATASET_SIZE, seed=SEED)
...
def train(device='cpu', T=500, img_size=224, input_channels=3, channels=32, time_dim=256,
          batch_size=2, lr=1e-3, num_epochs=400, experiment_name="ddpm", show=False):
    """Implements algrorithm 1 (Training) from the ddpm paper at page 4"""
    create_result_folders(experiment_name)
    dataloader = prepare_dataloader1(batch_size)

    model = UNet1(img_size=img_size, c_in=input_channels, c_out=input_channels, 
